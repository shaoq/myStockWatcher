...
## 项目简介
本项目是一个基于移动平均线（MA）的股票价格预警系统，帮助用户管理股票并实时监控价格是否达到均线目标。系统支持 A股（含北交所）和美股市场，提供智能缓存、交易时间感知、每日报告和历史数据分析功能。
...
### 每日报告
- **报告概览**: 监控总数、达标数量、达标率统计
- **状态变化追踪**: 新增达标 / 跌破均线 股票列表
- **趋势分析**: 近 7 日达标趋势图表
- **历史报告查看**: 支持选择历史日期查看报告
- **无报告引导**: 选择无快照的历史日期时，弹出对话框询问是否生成

### 交易日历
- **智能交易日判断**: 自动识别交易日/周末/节假日
- **多层数据源**: AkShare 主数据源 + exchange_calendars 备用源 + 周末兜底
- **日期格式兼容**: 支持多种日期格式解析
- **交易日状态显示**: 前端显示"交易日"或"休市"状态

### 数据来源
- **实时行情**: 新浪财经 API（hq.sinajs.cn）
- **K线数据**: 新浪财经 K 线接口
- **交易日历**: AkShare + exchange_calendars（双重保障）
...
| 数据库 | SQLite |
| 数据源 | 新浪财经 API |
| 交易日历 | AkShare + exchange_calendars |
...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/trading-calendar/check` | 检查指定日期是否为交易日 |
| POST | `/trading-calendar/refresh` | 刷新交易日历缓存 |

### 快照管理
| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/snapshots/generate` | 生成快照（支持指定历史日期） |
| GET | `/snapshots/check-today` | 检查今日是否有快照 |
| GET | `/snapshots/dates` | 获取所有有快照的日期列表 |

### 每日报告
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/reports/daily` | 获取每日报告（支持指定日期） |
| GET | `/reports/trend` | 获取趋势数据（最近 N 天） |
